...
class Session{
	...
	private function handleEncapsulatedPacket(EncapsulatedPacket $packet) : void{
		...
		if(PacketReliability::isSequencedOrOrdered($packet->reliability) and ($packet->orderChannel < 0 or $packet->orderChannel >= self::CHANNEL_COUNT)){
			// Syrim: el comportamiento de banning por peers excesivos se maneja en Network->blockAddress(). RakLib sólo reporta, no banea.
	...
	private function handleEncapsulatedPacketRoute(EncapsulatedPacket $packet) : void{
		...
		if($id < MessageIdentifiers::ID_USER_PACKET_ENUM){ //internal data packet
			if($this->state === self::STATE_CONNECTING){
				...
				// Syrim: al disconnect, RakLib v113 no envía ACK - se elimina la session inmediatamente. El cliente maneja el timeout por su lado.
	...
	/**
	 * @param int $sendPongTime Syrim: timestamp del último ping enviado, para calcular RTT. No se usa para clock sync en v113.
	 */
	...
	public function close() : void{
		if($this->state !== self::STATE_DISCONNECTED){
			...
			// Syrim: el ACK del client disconnect se ignora para evitar log spam. Es seguro - la session ya está marcada como closed.
