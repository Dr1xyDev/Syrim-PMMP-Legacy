...
class Installer {
        ...
        public function run(){
                ...
                if(strtolower($this->getInput()) === "y"){
                        // Syrim: aún con skip, preguntamos el MOTD porque
                        // es un paso rápido y muy solicitado por el usuario.
                        $this->promptMotd();
                        return true;
                }
                echo "\n";
                ...
                // Syrim: paso final de MOTD (siempre se ejecuta, haya o
                // no skip del resto de pasos).
                $this->promptMotd();
        ...
        /**
         * Syrim: pide al usuario el MOTD y lo guarda en motd.yml.
         * Si el usuario responde "c" se omite y se deja el valor por defecto.
         */
        private function promptMotd(){
                echo "\n[*] " . $this->lang->get("motd_step_intro") . "\n";
                echo "[?] " . $this->lang->get("motd_prompt") . ": ";
                $input = trim($this->getInput("c"));

                if(strtolower($input) === "c" or $input === ""){
                        echo "[*] " . $this->lang->get("motd_skipped") . "\n";
                        return;
                }

                // Sanitizar saltos de línea y guardar como YAML
                $motd = str_replace(["\n", "\r"], [" ", ""], $input);

                $motdConfig = new Config(\pocketmine\DATA . "motd.yml", Config::YAML, [
                        "motd" => $motd
                ]);
                $motdConfig->set("motd", $motd);
                $motdConfig->save();

                echo "[*] " . $this->lang->get("motd_saved") . "\n";
                echo "[*] MOTD: " . $motd . "\n";
        }
