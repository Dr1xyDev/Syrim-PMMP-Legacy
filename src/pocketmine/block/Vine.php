...
class Vine extends Flowable{
    ...
    const FLAG_WEST = 0x02;
        ...
        protected function recalculateBoundingBox(){
                ...
                // Syrim: check NORTH (flag 0x04) - bounding box en cara Z=0.
        ...
        public function onUpdate($type){
                if($type === Level::BLOCK_UPDATE_NORMAL){
                        ...
                        // Syrim: crecimiento de vines - en MCPE vanilla las vines
                        // se extienden hacia abajo si la cara superior del bloque
                        // inferior está libre y la vine actual tiene soporte.
                        // Implementación simplificada: 25% de probabilidad de
                        // extenderse un bloque hacia abajo si hay aire abajo y
                        // la vine actual tiene un soporte sólido arriba.
                        if(mt_rand(0, 3) === 0){
                                $down = $this->getSide(Vector3::SIDE_DOWN);
                                if($down->getId() === Block::AIR){
                                        $up = $this->getSide(Vector3::SIDE_UP);
                                        if($up->isSolid() or ($up instanceof Vine)){
                                                $this->level->setBlock($down, Block::get(Block::VINE, $this->meta), true);
                                                return Level::BLOCK_UPDATE_NORMAL;
                                        }
                                }
                        }
