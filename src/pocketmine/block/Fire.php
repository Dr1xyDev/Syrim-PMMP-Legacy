        ...
        public function onUpdate($type){
                if($type == Level::BLOCK_UPDATE_NORMAL or $type == Level::BLOCK_UPDATE_RANDOM or $type == Level::BLOCK_UPDATE_SCHEDULED){
                        if(!$this->getSide(Vector3::SIDE_DOWN)->isTopFacingSurfaceSolid() and !$this->canNeighborBurn()){
                                ...
                                // Syrim: el manejo de fuego en el End se omite porque en MCPE v113
                                // el End no tiene mecánicas especiales de fuego (no hay bedrock
                                // que lo sostenga infinitamente como el netherrack del Nether).
                                // El bloque inferior ya se valida con isTopFacingSurfaceSolid().
                                ...
                                if(!$forever and $this->getLevel()->getWeather()->isRainy() and
                                        ...
                                        if(!$forever and !$this->canNeighborBurn()){
                                                ...
                                                // Syrim: en MCPE v113 no hay biome-rainfall data por
                                                // bloque (sólo un flag global de lluvia). El ajuste de
                                                // o por rainfall se aplica arriba cuando canBlockSeeSky
                                                // + rainy apaga el fuego. Aquí el $o se mantiene en 0.
                                                        ...
                                                        for($z = ($this->z - 1); $z <= ($this->z + 1); ++$z){
                                                                for($y = ($this->y - 1); $y <= ($this->y + 4); ++$y){
                                                                        ...
                                                                        if($chance > 0){
                                                                                ...
                                                                                // Syrim: igual que arriba, el ajuste por rainfall se maneja
                                                                                // globalmente con canBlockSeeSky + isRainy. No hay rainfall
                                                                                // data por bloque en MCPE v113.
