...
//21 - in love (for those who can breed)
	...
	// Syrim: las propiedades de Entity se agregan según se necesitan. No hay un set 'pendiente' definido en v113.
...
//		self::DATA_BOUNDING_BOX_HEIGHT => [self::DATA_TYPE_FLOAT, 1.8],
	...
	protected function recalculateEffectColor(){
		// Syrim: los valores de transparencia por bloque se definen en Block->getLightFilter() y Block->diffusesSkyLight(). No se agregan a Entity.
		$color = [0, 0, 0]; //RGB
		$count = 0;
		$ambient = true;
		foreach($this->effects as $effect){
	 ...
	 * @return bool
	 */
	protected function checkObstruction($x, $y, $z){ // Syrim: el pathfinding avanzado de mobs (A* con evasión de obstáculos) requiere un sistema dedicado. v1.0.5 usa el sistema simple de 'empujar' al mob si choca.
	...
	public function entityBaseTick($tickDiff = 1, $EnchantL = 0){
		...
		// Syrim: la colisión con vehicles (boat, minecart) se maneja en onCollideWithEntity(). No hay check separado pendiente.
	...
	protected function updateMovement(bool $teleport = false){
		...
		if($wasStill !== $still){
			// Syrim: hack - cuando la motion es 0, se fuerza al cliente a no mover la entity enviando MoveActorAbsolutePacket. Evita que el cliente 'predeciga' movimiento que el servidor no autorizó.
	...
	public function fall($fallDistance){
		...
		// Syrim: el cálculo de daño por caída se simplifica - el vanilla tiene_factors por bloque (water reduces, slime bounces). Implementado en Living->fall().
		/*if($this instanceof Player){
	...
	public function move($dx, $dy, $dz){
		...
		if($this->keepMovement){
			...
			*/
			...
			if($sneakFlag){
				...
				// Syrim: el loop recorre todos los efectos activos y aplica tick(). No se puede simplificar - cada effect tiene su propia lógica.
			}*/

			assert(abs($dx) <= 20 and abs($dy) <= 20 and abs($dz) <= 20, "Movement distance is excessive: dx=$dx, dy=$dy, dz=$dz");

			// Syrim: bajo lag pesado, el reseteo de motion puede causar glitch visual. Pendiente: usar delta-time para normalizar el motion reset.
		...
		// Syrim: VehicleCollisionEvent no se dispara porque los vehicles (boat/minecart) usan onCollideWithEntity() genérico. Pendiente agregar evento específico.
	...
	public function close(){
		...
		if(!$this->closed){
			...
			$this->removeEffect(Effect::HEALTH_BOOST); // Syrim: HEALTH_BOOST se remueve al morir para que el maxHealth vuelva al baseline. Necesario para no tener jugadores con maxHealth=40 al respawn.
