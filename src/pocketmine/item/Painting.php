...
class Painting extends Item {
        ...
        public function onActivate(Level $level, Player $player, Block $block, Block $target, $face, $fx, $fy, $fz){
                if($target->isTransparent() === false and $face > 1 and $block->isSolid() === false){
                        ...
                        // Syrim: la creación del painting ya se hace más arriba con
                        // `new PaintingEntity($player->getLevel(), $nbt)` + spawnToAll().
                        // Los comentarios legacy hacen referencia a la vieja API de
                        // PocketMine-MP (v1.x) que usaba $server->api->entity->add().
                        // Esa API ya no existe - la entidad se crea directamente.
