	...
	public static function growTree(ChunkManager $level, $x, $y, $z, Random $random, $type = 0){
		switch($type){
			...
			case Sapling::BIRCH:
				...
				break;
			case Sapling::JUNGLE:
				// Syrim: el JungleTree ahora tiene 3 variantes:
				//   - 80% -> NewJungleTree (jungle tree pequeño con vines + cacao)
				//   - 15% -> ObjectJungleBigTree (2x2 tronco, gran corona)
				//   -  5% -> JungleTree (legacy, simple)
				$r = $random->nextBoundedInt(20);
				if($r < 16){
					$tree = new NewJungleTree(4);
				}elseif($r < 19){
					$tree = new ObjectJungleBigTree(5, 5, Wood::JUNGLE, Leaves::JUNGLE);
				}else{
					$tree = new JungleTree();
				}
				break;
			case Sapling::DARK_OAK:
				// Syrim: dark oak tree vanilla port de Nukkit (2x2 tronco + copa robusta)
				$tree = new ObjectDarkOakTree();
				break;
			case Sapling::OAK:
			default:
				$tree = new OakTree();
