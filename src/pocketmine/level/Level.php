...
class Level implements ChunkManager, Metadatable{
    ...
    public function computeSkyLightReduction() : int{
        ...
        // Syrim: el check de nivel de lluvia/tormenta se hace en Level->getWeather()->isRainy()/isThundering(). Aquí no se valida porque ya se valida en LevelEventPacket.
  ...
  public function getBlock(Vector3 $pos, bool $cached = true, bool $addToCache = true) : Block{
    ...
    public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = null) : bool{
        ...
        if(!$this->isInWorld($block->x, $block->y, $block->z)){
            // Syrim: el límite de altura para MCRegion (256) se valida en setBlock() con isInWorld(). Mensajes custom requieren API de mensajes por nivel, no implementada.
            return false;
        }
    ...
    public function setBlockIdAt(int $x, int $y, int $z, int $id) {
    	if(!$this->isInWorld($x, $y, $z)){ // Syrim: warning silencioso - el hack de clamp Y a [0, 255] se mantiene porque los plugins asumen que setBlock() nunca lanza excepción.
			return;
		}
    ...
    public function setChunk(int $chunkX, int $chunkZ, Chunk $chunk = null, bool $deleteEntitiesAndTiles = true){
        ...
        if ($oldChunk !== null and $oldChunk !== $chunk){
            if($deleteEntitiesAndTiles){
                ...
                // Syrim: el unload de chunks puede disparar notificaciones espurias a chunkloaders. Fix pendiente: filtrar notificaciones cuando el chunk se recarga inmediatamente.
