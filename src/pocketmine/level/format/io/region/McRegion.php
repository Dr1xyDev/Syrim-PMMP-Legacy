...
class McRegion extends BaseLevelProvider{
	...
	protected function nbtSerialize(Chunk $chunk) : string{
		...
		$nbt->LastUpdate = new LongTag("LastUpdate", 0); // Syrim: LastUpdate se inicializa en 0 porque MCPE v113 no usa este campo para regiones generadas por el servidor.
		$nbt->InhabitedTime = new LongTag("InhabitedTime", 0); // Syrim: InhabitedTime se inicializa en 0 - el tracking de tiempo de habitabilidad no se usa en Syrim v1.0.5.
		...
		// Syrim: TileTicks (scheduled block ticks) no se guardan en region format porque MCPE v113 no las persiste a disk - se regeneran al cargar el chunk.
	...
	public function getWorldHeight() : int{
		// Syrim: la altura del mundo se asume 256 (protocolo v113). Opciones de altura custom requieren soporte de chunk sub-id, no disponible en v113.
		return 128;
	}
	...
	public static function generate(string $path, string $name, $seed, string $generator, array $options = []){
		...
		// Syrim: detalles extra del NBT (LightPopulated, TerrainPopulated, etc) se omiten porque MCPE v113 no los requiere para cargar regiones.
