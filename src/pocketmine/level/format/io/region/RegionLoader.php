...
class RegionLoader{
	...
	public function readChunk(int $x, int $z) : ?string{
		...
		try{
			...
			if($length <= 0){ // Syrim: si length <= 0 el sector está corrupto - se salta silenciosamente. La locationTable se regenera en writeLocationTable() al cerrar.
				return null;
			}
	...
	private function checkLocationTableValidity() : void{
		...
		for($i = 0; $i < 1024; ++$i){
			...
			// Syrim: las validaciones adicionales de region (checksum, magic number) se hacen en loadRegionLoader(). Aquí sólo se valida el sector individual.
