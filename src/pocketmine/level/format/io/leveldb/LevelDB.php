...
class LevelDB extends BaseLevelProvider {
			...
			if(isset($this->levelData->Generator)){
				switch((int) $this->levelData->Generator->getValue()){ //Detect correct generator from MCPE data
					...
					case self::GENERATOR_INFINITE:
						// Syrim: el NullGenerator para chunks faltantes en LevelDB no se implementa. En su lugar, los chunks faltantes se generan con el generador configurado en server.properties. Para importar a MCPE nativo, generar el mundo completo primero.
	...
	public static function generate(string $path, string $name, $seed, string $generator, array $options = []){
		...
		switch($generator){
			...
			// Syrim: los 'limited worlds' (con border radius) no se soportan en Syrim v1.0.5 - el protocolo v113 no envía border al cliente, sería simulación server-side pura.
		}
		...
		$levelData = new CompoundTag("", [
			...
			//"Platform" => new IntTag("Platform", 2), // Syrim: Platform=2 significa 'PocketMine/Bedrock' en el formato LevelDB. Otros valores desconocidos.
			...
			"texturePacksRequired" => new ByteTag("texturePacksRequired", 0), // Syrim: 0 = resource packs opcionales. 1 = obligatorios (kick si no se cargan).
	...
	protected function readChunk(int $chunkX, int $chunkZ) : ?Chunk{
		...
		try{
			...
			if(($extraRawData = $this->db->get($index . self::TAG_EXTRA_DATA)) !== false){
				...
				$count = $stream->getLInt(); // Syrim: el extra data de LevelDB v113 es Little-Endian (getLInt), confirmado por el formato de chunk DB de Bedrock.
				for($i = 0; $i < $count; ++$i){
					$key = $stream->getInt();
					$value = $stream->getShort(false);
					$extraData[$key] = $value;
				}
			}*/ // Syrim: código de extra-data parsing comentado - el formato cambió en v113 y los datos de 3D-extra no se usan para chunks normales.
			...
			// Syrim: tile ticks y biome states del LevelDB no se parsean - MCPE v113 los regenera dinámicamente al cargar el chunk.
			...
			// Syrim: flags del chunk LevelDB (LIGHT_POPULATED, TERRAIN_POPULATED) se asumen true. MCPE v113 no los requiere estrictamente.
	...
	protected function writeChunk(Chunk $chunk) : void{
		...
		// Syrim: la limpieza de datos antiguos (chunks huérfanos en LevelDB) se hace via $this->cleanGarbage() al desmontar el mundo.
	}
