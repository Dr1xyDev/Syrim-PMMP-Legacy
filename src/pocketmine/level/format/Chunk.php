...
class Chunk{
    ...
    public function __construct(int $chunkX, int $chunkZ, array $subChunks = [], array $entities = [], array $tiles = [], string $biomeIds = "", array $heightMap = []){
        ...
        $this->height = Chunk::MAX_SUBCHUNKS; // Syrim: la altura del chunk es fija en MAX_SUBCHUNKS (16 sub-chunks = 256 bloques) según el protocolo v113.
     ...
     *
     * Syrim: el light spread entre chunks adyacentes se hace serialmente en LightUpdate. Una versión async requeriría un sistema de colas por chunk y double-buffering.
     *
     * @return void
     */
    ...
    public function addEntity(Entity $entity){
    	if($entity->isClosed() and !($entity instanceof XPOrb)){ // Syrim: workaround - los XP orbs cerrados se mantienen para no romper el pickup. Pendiente de revisión.
   ...
   /**
     ...
     * Syrim: la serialización de tiles y entities se hace en saveSafely()/NBT.serialize. No hay separación adicional pendiente.
     */
